...
def func(x, a, b, c):
#    return a * bigfloat.exp(-b * x,bigfloat.precision(100)) + c
...
colAmpl=int(4)-1	# column in infile with amplitude values
colEW=int(5)-1	# column in infile with echo width values
try:
	numPtsUsed=int(10000000)	# number of points to use as a (random) subset of the input point cloud data
except:
...
try:
    ...
    popt, pcov = curve_fit(func,  xdata, ydata,  p0=(0, -1, 0))
except:
...
popt_mirr, pcov_mirr = curve_fit(func, xdata_r, ydata_r)
...
#ax.scatter(xdata, ydata, color="cyan", s=15.0,  alpha=0.4, label='Data for fitting')
...
plt.plot(xdata, func(xdata, *popt), 'r-', label='Curve fit to left flank', linewidth=0.9)

# plot right side curve (mirrored around median)
plt.plot(xdata_r, func(xdata_r, *popt_mirr), 'r--', label='Curve flipped along mean', linewidth=0.9)

# plot shifted curve (transition zone)
plt.plot(xdata_r_shifted, func(xdata_r_shifted, *popt_mirr_shft), color='orange', linestyle='dashed', label='Curve delineating transition zone', linewidth=0.9)

plt.fill_between(xdata, 0, func(xdata, *popt),color="red",alpha=0.25, edgecolor='none')
plt.fill_between(xdata_r, 0, func(xdata_r, *popt_mirr),color="red",alpha=0.25, edgecolor='none')
...
#plt.show()
figname="05_plots/scatter_terrain_probability1.png"
...
c=0
p=5
outfile= "02_intermediate/" + basename + "_terrainprob.txt"
with open(outfile,"w") as f:
    with open(infile,"r") as ifo:
        ...
        for line in ifo:
            ...
            curveval_left=func(echowidth, *popt)
            curveval_right=func(echowidth, *popt_mirr)
            curveval_trans=func(echowidth, *popt_mirr_shft)
